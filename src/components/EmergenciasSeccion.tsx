...
// Llamadas a ambulancia / emergencias de la sede (Legales → Certificaciones y proveedores).
export async function EmergenciasSeccion({
  sucursalId,
  anio,
  imprimible = false,
}: {
        ...
        {!imprimible && (
          <div className="flex items-center gap-2">
            {[anioActual - 1, anioActual].map((a) => (
              <Link
                key={a}
                href={`/sucursales/${id}/legales/certificaciones?anio=${a}`}
            ...
            <Link
              href={`/sucursales/${id}/legales/emergencias?anio=${anio}`}
              className="rounded-lg border border-edge px-3 py-1.5 text-xs font-semibold text-ink-soft hover:border-brass hover:text-ink"
            >
              Informe (PDF)
            </Link>
          </div>
