...
export default async function InformeDeudoresGeneralPage() {
  const supabase = await createClient();

  const [{ data: sucursales }, { data: residentes }, { data: pagos }] = await Promise.all([
    ...
    supabase
      .from("residentes")
      .select(
        "id, nombre, apellido, sucursal_id, ficha_administrativa(obra_social, porcentaje_recargo_mora, fecha_vencimiento_cuota)",
      )
      .eq("activo", true)
      .order("apellido")
  ...
  const deudores = (residentes ?? [])
    .map((r) => {
      ...
      return {
        ...
        nombre: r.nombre,
        apellido: r.apellido,
        sede: nombreSede.get(r.sucursal_id) ?? "—",
          ...
          <tbody>
            {deudores.map((d) => (
              <tr key={d.id} className="border-b border-edge print:border-neutral-300">
                ...
                <td className="py-2.5 pr-3 text-ink-soft print:text-neutral-700">{d.sede}</td>
