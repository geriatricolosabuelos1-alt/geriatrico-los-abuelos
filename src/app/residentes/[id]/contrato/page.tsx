...
export default async function ContratoResidentePage({
  ...
  const [{ data: sucursal }, { data: fichaAdministrativa }, { data: perfil }, { data: contratoEditado }] =
    await Promise.all([
    supabase
      .from("sucursales")
    ...
    <div className="flex min-h-screen w-full justify-center bg-panel px-4 py-10 print:block print:min-h-0 print:bg-white print:px-0 print:py-0">
      ...
      <div className="w-full max-w-[760px] space-y-5 rounded-2xl border border-edge bg-card p-10 text-sm leading-relaxed text-ink shadow-2xl print:max-w-none print:rounded-none print:border-0 print:bg-white print:p-0 print:text-black print:shadow-none">
        ...
        <ContratoEditable
          ...
          editado={!!contratoEditado}
        >
          {contratoEditado ? (
            contratoEditado.parrafos.map((texto, i) => {
              if (i === 0) {
                return (
                  <h1
                    key={i}
                    className="text-center font-display text-lg font-bold uppercase text-ink print:text-black"
                  >
                    {texto}
                  </h1>
                );
              }
              const clausula = texto.match(/^(CLÁUSULA [^:]{1,40}:)(.*)$/);
              return (
                <p key={i}>
                  {clausula ? (
                    <>
                      <span className="font-semibold">{clausula[1]}</span>
                      {clausula[2]}
                    </>
                  ) : (
                    texto
